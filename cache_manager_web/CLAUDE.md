...
| POST | /api/capture | Receive capture from extension (`html` is converted with `html_to_markdown()`; `text` is stored as is when no HTML is sent; `visible_part_only` flags the page; with `batch`, a batch capture: stored only while the batch waits for `url`, else 409 and nothing stored) |
...
| POST | /api/upload-pdf/{id} | Upload PDF (replaces content, switches type; 422 without the `%PDF-` signature; `batch` query parameter as for `/api/capture`) |
...
- **Only a batch capture advances the batch**: the extension sends its captures and PDF uploads with `batch`; the backend stores them only while their URL is the head of the queue, gives them the `"recaptured"` status, and pops the head; otherwise it refuses them with 409 and stores nothing (`_refuse_a_stale_batch_capture()`). A skip names its URL and is checked the same way. Captures made by hand are not checked. The batch does not set the capture target, so the extension's single capture keeps capturing for the URL selected in the UI

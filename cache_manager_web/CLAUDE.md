...
| POST | /api/capture | Receive capture from extension (`url` must be http(s), else 400; the page is stored once, with `actual_url`, the URL the tab ended at, recorded as its final URL unless it is not http(s) or is on the server's host; 413 above the size limits; `html` is converted with `html_to_markdown()`; `text` is stored as is when no HTML is sent; `visible_part_only` flags the page, and a capture by hand with empty text is left unreviewed, both with a `warning`; with `batch`, a batch capture: stored only while the batch waits for `url`, else 409 and nothing stored) |
...
| POST | /api/upload-pdf/{id} | Upload PDF (replaces content, switches type; 422 without the `%PDF-` signature; 400 unless `url` is http(s); 413 above `MAX_UPLOAD_SIZE`; `batch` query parameter as for `/api/capture`; an `actual_url` query parameter, which the extension sends when the PDF came from another URL, is recorded as the page's final URL) |
...
- **URL redirect handling**: a capture is stored once, for the URL selected, and its `actual_url` is recorded as the page's final URL (`CacheFileSys.put_web(..., final_url=...)`), so the final URL is not listed as a separate entry and resolves to the captured page; a pending or failed entry for the final URL stops being listed while the redirect resolves, but stays in `pending.json` or `failures.json`, so it is listed again if the captured page is deleted or reset. A URL that only resolves through a redirect record is not an entry: the per-URL edit routes answer 404 for it (`_listed_url()`, `CacheManager.resolves_through_redirect()`), so an edit never acts on the captured page by its final URL; a capture or upload named by it stores it as its own page (`store_page()` resolves with `canonical_url(..., follow_redirects=False)`), which takes the redirect record's place; and Add and Rename answer 409 for it, since it already resolves to a page. Rename moves the page's redirect record to the new URL. No redirect is recorded for a final URL that has a stored page in its own letter case, and a page stored under a URL that differs from the final URL only in letter case stays a separate page, so a redirect never replaces a listed page

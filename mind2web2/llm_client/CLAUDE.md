...
Chat Completions client used by the judge (and by `mind2web2 cache` for URL extraction).
...
- `JudgeError`: a judge request failed for good (non-retryable error, or transient errors beyond the retry budget). Evaluation code must let it propagate: the answer is then reported as not scored and evaluated again on the next run. After one failed request, the answer's later requests fail without being sent.
...
- `JudgeUsage`: requests, failed requests, rejections, page loads failed because of the environment (`harness_failures`), the models the server reported (`served_models`), and token counts (input, cached input, output, reasoning) for one answer; recorded in the result as `judge_usage`.

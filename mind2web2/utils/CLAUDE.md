...
**Key methods:**
- `put_web(url, text, screenshot, final_url=None)` / `put_pdf(url, pdf_bytes, final_url=None)`: store a page under `storage_key(url)` (fragment removed, percent-decoded, trailing slash removed), replacing any page under the same key whatever its type; return the page's URL as `lookup` returns it. `final_url` is the URL the content was served at after redirects; see **Redirects** below.
- `get_web(url)` → `(text, jpeg_bytes)`; `get_pdf(url)` → `pdf_bytes`; `has(url)` → `"web"` | `"pdf"` | `None`
- `lookup(url, ignore_case=True, follow_redirects=True)` → the URL of the cached page `url` refers to, or `None`; `get_all_urls()` lists these URLs; `remove(url)` deletes a page and its files. A URL from `lookup` or `get_all_urls` passed to any method addresses the same page.
- `redirects()`: the recorded final URLs that `lookup` resolves, each with its page's URL.
...
3. a surface variant of `url` is the key (scheme, `www.`, UTM suffixes, percent-encoding forms, trailing slash). Runs only when 1-2 miss; it finds keys whose normalized form changes under percent-decoding.
4. the key has the same lowercased form `normalize_url_simple(url)`

Rules 1-3 respect letter case and rule 4 disregards it, so that, among pages stored for URLs that differ only in letter case (which a server may serve as different pages), a URL finds the one with its own letter case, and another only when there is none. `lookup(url, ignore_case=False)` and `failure(url, ignore_case=False)` skip rule 4, for callers such as the crawler that must not take a page of another letter case for this one; recording or clearing a failure always matches that way. Pages, failure records, and redirect records are matched by one implementation, `_UrlIndex`.

**Redirects.** A page whose capture ended at another URL (its final URL: another `page_form`, http(s)) is stored once, under the URL requested, and `redirects.json` maps the final URL to it. When rules 1-3 find no page, they are applied to the recorded final URLs; rule 4 is then applied to pages, and after that to final URLs. A redirect recorded in the query's letter case therefore wins over a page stored under another letter case. A final URL is not recorded when rules 1-3 find a page for it (the on-disk index included); one final URL has one record, so the latest capture that ended at it, in any case-respecting spelling, answers for it; storing a page deletes the records whose final URL rules 1-3 match to it, storing a page again deletes the records pointing to its previous capture, and removing a page deletes the records pointing to it. A final URL's failure record is kept: it is ignored while the final URL resolves to a page, and applies again once that page is removed (`remove` deletes only the failure records the page itself hides). `lookup(url, follow_redirects=False)` finds only a page stored for `url` itself, for callers that delete or change what they find.
...
- `capture(url, logger)` → `Capture`: `screenshot_b64` (PNG), `text` (the HTML converted to Markdown with `html2text`), and `final_url` (the URL of the page captured, after redirects) on success; otherwise `error`, with `blocked=True` when the site refused the browser.
...
- `configure_run_logging(log_dir, name, console=True)` / `close_run_logging()`: a command's run log, `<log_dir>/<timestamp>_<name>.log` and `.jsonl`, fed by the `mind2web2` package logger, plus the console (stderr, through `tqdm.write` so that it prints above progress bars). A record with `extra={"console": False}` goes to the files only. WARNING and higher records of loggers outside the package (the OpenAI SDK, the browser library, an eval script's module logger) are routed the same way, through a handler on the root logger. `close_run_logging()` removes that handler and restores the package logger's level and propagation.
- `logging_to(logger)`: inside the block (and in asyncio tasks created in it and functions it runs with `asyncio.to_thread`, not in threads started with `threading.Thread`), records of the package's module loggers go to `logger`, the answer's log, instead of the run log.

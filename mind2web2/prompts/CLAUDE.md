...
### cache_prompts.py
Contains `llm_extraction_prompts`: A system prompt used by `LLMUrlExtractor` in `crawl.py` to extract URLs from answer text via LLM. The LLM is instructed to find all unique website URLs in the text, including hard-to-find URLs like bare domains, URLs in quotes, and URLs split across lines.

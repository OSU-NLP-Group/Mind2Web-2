...
`mind2web2 evaluate` also writes `eval_results/<your_agent_name>/report.html`, a single HTML file to open in a browser; `uv run mind2web2 report <your_agent_name>` writes it again from the saved results (`--task` limits it to given tasks, `--output` names another file). It shows the saved metrics, a table of every task's score in every run, and for each answer its rubric tree with each node's status and score. Each check the eval script verified shows its evidence: the claim, the pages it was checked against, the judge's votes, and its reasoning, or the check it depended on when it was skipped; failed and skipped checks show it open. The extracted information and the answer text follow. The evidence is part of each result file (`evidence` on the leaves of `verification_tree`), so results saved before it was recorded show the tree without it until they are evaluated again with `--overwrite` (the evidence is not part of what decides whether a result is reused). The page loads nothing from the network and runs only its own script, so it can be opened from disk and shared as a file.

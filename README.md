...
Each task's eval script scores every `answer_<k>.md` of the task, and each result is saved under `eval_results/<your_agent_name>/<task_id>/`. Eval scripts are released in version directories named by date, such as `evaluation_scripts/2025_10_23/`; the newest version is used unless `--eval-version` names another. Pages come from the cache, and a page missing from it is captured live and added to it. Answers that already have a result are skipped unless `--overwrite`, so after an interruption or a failure, running the command again evaluates only the answers without a result. The command ends with one line per task (answers scored, mean score) and the metrics of the next section, and it exits with status 1 if some answer has no result.

...
- A run shares one browser for live captures: `mind2web2 evaluate` wraps the run in `eval_toolkit.shared_browser()`, `eval_runner.evaluate_task()` and `evaluate_tasks()` share one through `eval_toolkit.browser_for_run()` when none is shared, and evaluators created inside use that browser instead of starting their own
